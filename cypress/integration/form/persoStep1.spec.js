context('Form | À la maison step', () => {
    ...
    it('should show à la maison step on form page', () => {
        ...
        cy.stubRequest('GET', '**/api/response/thematic/**', 200, 'form/persoStep1/state_with_details.json', 'getResponsesOfStep5');
        sizes.forEach((size) => {
    ...
    it('should get responses when user respondes with "non" to the question "Souhaitez-vous entrer dans le détail de vos consommations énergétiques ?"', () => {
        cy.stubRequest('GET', '**/api/user/progress', 200, 'form/persoStep1/progress.json', 'progressJSON');
        cy.stubRequest('GET', '**/api/response/thematic/**', 200, 'form/persoStep1/state_without_details.json', 'getResponsesOfStep5');
        cy.window().then((win) => {
            win.sessionStorage.clear();
            cy.login('email@paris2024.org');
            cy.visit('/form');
            ...
            cy.get('#5f55608002862').parents('.ant-select-selector').find('.ant-select-selection-item').should('have.attr', 'title', 'B = 70,5');
            ...
            cy.get(`body:contains("Quel(s) type(s) d'énergie utilisez-vous ?")`)
                .should("not.exist");
            cy.get(`body:contains("Votre chauffage ou eau chaude sanitaire dépend-il d'un système collectif ?")`)
                .should("not.exist");
        });
    });
	...
	it('should get responses when user respondes with "oui" to the question "Souhaitez-vous entrer dans le détail de vos consommations énergétiques ?"', () => {
		...
		cy.window().then((win) => {
            ...
            ['Electrique', 'Gaz', 'Fioul', 'Bois'].forEach(energy => cy.get('@checkedEnergies').find(`input[value=${energy}]`).should('exist'));
            ...
            cy.get('#to-insert-6').find('label.ant-radio-button-wrapper-checked').contains('Chauffage collectif').should('exist');
            cy.get('#5fe468b6e6a06').find('label.ant-radio-button-wrapper-checked').contains('Fioul').should('exist');

            cy.get(`body:contains("Votre électricité et gaz sont-ils renouvelables ?")`)
                .should("not.exist");
        });
	});
});
