...
import {
	DEPLACEMENTS_PROFESSIONNEL_OVERLAY_FRANCE_TITLE,
	DEPLACEMENTS_PROFESSIONNEL_OVERLAY_INTERNATIONNAL_TITLE,
	...
	QUESTION6_NBR_TRAJETS_AR_TOOLTIP,
} from '@utils/constants';
import { saveResponsesOfStep, getResponsesOfStep } from '@services/responseService';
...
export function ProStep5({ step, setNextStep }) {
	...
	const [isTransportationQuestionShown, setTransportationQuestionShown] = useState(false);
	...
	const trajetsARDynamicProps = [
		{
			...
			name: '5f55582edaeda',
			tooltipTitle: QUESTION6_NBR_TRAJETS_AR_TOOLTIP,
		},
		{
	...
	useEffect(() => {
		...
		const setReponsesOfStep = (stepState) => {
			...
			if (form.getFieldValue('5fe088aa8d674')) {
				setCarQuestionsVisible(form.getFieldValue('5fe088aa8d674').includes('Voiture'));
				setTrainQuestionsVisible(form.getFieldValue('5fe088aa8d674').includes('Train'));
				setPlaneQuestionsVisible(form.getFieldValue('5fe088aa8d674').includes('Avion'));
			}
		};
	...
	return (
		<ConfiguredForm id={step} form={form} onFinish={onFinish} onFinishFailed={() => console.log('onFinishFailed')}>
			<div className="wizard-content-right-form-parent">
				...
				{isTransportationQuestionShown && (
					<div className="forms-margin">
						...
						/>
						<Overlay title={DEPLACEMENTS_PROFESSIONNEL_OVERLAY_FRANCE_TITLE} items={overlay_items_france} />
						<Overlay
							title={DEPLACEMENTS_PROFESSIONNEL_OVERLAY_INTERNATIONNAL_TITLE}
				...
				<div className="wizard-content-right-form-parent">
					...
					{trajetsARIndex.map((index, key) => (
						<div className="forms-margin" key={key}>
							<FormItemInputNumber form={form} {...trajetsARDynamicProps[key]} />
						</div>
					))}
				</div>
			)}
