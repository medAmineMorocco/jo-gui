...
export function ProStep5({ step, setNextStep }) {
	...
	useEffect(() => {
		...
		const setReponsesOfStep = (stepState) => {
			...
			setTransportationQuestionShown(form.getFieldValue('to-insert2'));
			setCarQuestionsVisible(form.getFieldValue('to-insert3').includes('Voiture'));
			setTrainQuestionsVisible(form.getFieldValue('to-insert3').includes('Train'));
			setPlaneQuestionsVisible(form.getFieldValue('to-insert3').includes('Avion'));
		...
		<ConfiguredForm id={step} form={form} onFinish={onFinish} onFinishFailed={() => console.log('onFinishFailed')}>
			<div className="wizard-content-right-form-parent">
				<div className="pro-step-title-container">
					<span className="pro-step-title">Déplacements professionnels sur l'année</span>
				</div>
				...
				{isTransportationQuestionShown && (
					<div className="forms-margin">
						...
						/>
						<Overlay title={DEPLACEMENTS_PROFESSIONNEL_OVERLAY_TITLE} items={overlay_items} />
					</div>
				)}
			</div>
			...
			{arePlaneQuestionsVisible && (
				<Fragment>
					<div className="wizard-content-right-form-parent">
						...
						<div className="forms-margin">
							<FormItemInputNumber form={form} label={QUESTION7_NBR_TRAJETS_AR} name="5f55584be6d5b" />
						</div>
